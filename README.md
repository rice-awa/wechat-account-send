...
使用 OpenClaw/Hermes Agent 复用的微信插件，实现微信账号信息查询、会话管理、主动文本消息和文件发送功能。
可以通过clawhub命令安装
```bash
...
本项目提供了一套完整的微信消息和文件发送解决方案，支持通过 OpenClaw 微信插件与微信后端API进行交互。该工具可以帮助开发者和用户实现自动化的微信消息和文件发送功能。并且同时支持 Hermes Agent 与 OpenClaw，两者使用的微信插件本质一致，脚本会自动发现两类环境下的账号配置。
